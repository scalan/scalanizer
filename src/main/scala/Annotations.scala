...
class Annotations(val global: Global)
  ...
  val runsAfter = List[String]("scalan")
  override val runsRightAfter: Option[String] = Some("scalan")
  ...
  class AnnotTransformer extends Transformer {
    ...
    override def transform(tree: Tree): Tree = tree match {
      case q"object StagedEvaluation {..$body}" =>
        val newBody = body.map(stat => stat match {
